...
class ImportDataDialog(QtWidgets.QDialog, FORM_CLASS):  # type: ignore
    ...
    def __init__(self, df, a_b, dlg, parent=None):
        ...
        tv_df_widths = [130, 340, 60, 60, 50, 50, 50]
        tv_idx_widths = [70, 50]

        # Utworzenie modeli dla tableview'ów:
        self.df_mdl = IDfModel(df=df, tv=self.tv_df, col_widths=tv_df_widths)
        self.mdl_in = IDfModel(df=self.df_in, tv=self.tv_idx_in, col_widths=tv_idx_widths)
        self.mdl_out = IDfModel(df=self.df_out, tv=self.tv_idx_out, col_widths=tv_idx_widths)
        ...
        # Parametry:
        all_params = [
            {'param' : 'Z', 'label' : 'Z', 'df_in' : self.z_in, 'df_out' : self.z_out, 'type_in' : self.type_in_z, 'type_out' : self.type_out_z, 'type_act' : self.type_act_z, 'done' : self.done_z, 'btn' : self.btn_param_z, 'light' : self.light_z},
            {'param' : 'H', 'label' : 'GŁ.', 'df_in' : self.h_in, 'df_out' : self.h_out, 'type_in' : self.type_in_h, 'type_out' : self.type_out_h, 'type_act' : self.type_act_h, 'done' : self.done_h, 'btn' : self.btn_param_h, 'light' : self.light_h},
            {'param' : 'ROK', 'label' : 'ROK', 'df_in' : self.r_in, 'df_out' : self.r_out, 'type_in' : self.type_in_r, 'type_out' : self.type_out_r, 'type_act' : self.type_act_r, 'done' : self.done_r, 'btn' : self.btn_param_r, 'light' : self.light_r}
            ]
        ...
        for dicts in self.params:
            for key, value in dicts.items():
                ...
                if key == 'param' and value == val:
                    ...
                    label = dicts['label']
        self.old_param = val
        self.lab_act_param.setText(label)
